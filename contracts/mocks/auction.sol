  ...
  Ownable
{
  ...
  function refund(
    ...
    internal
  {
    //uint freezeVal = TokenDMA(token20).allowance(_refundAddress, address(this));
    if(currentValue > 0){
      TokenDMA(token20).transfer(_refundAddress, currentValue);
    }
  }
  ...
  function updateBidInfo(
    ...
    internal
  {
    uint approveVal = TokenDMA(token20).allowance(_newAddress, address(this));
    
    require(approveVal >= _newValue, "approve value of new adderss isn't enough");
    
    TokenDMA(token20).transferFrom(_newAddress, address(this),_newValue);
    ...
    internal
  {
    ...
    TokenDMA(token20).transferFrom(address(this), tokenOwner, currentValue);
